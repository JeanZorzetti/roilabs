---
title: 'Mercado Livre, Shopee ou canal próprio: onde o fornecedor regional vende sem perder a margem'
description: Marketplace é excelente para produto leve, padronizado, de alto giro e margem folgada. Para o fornecedor regional de produto pesado, sob medida ou de venda consultiva, a comissão, o frete e a devolução comem a margem — e o cliente fica com o canal. Este artigo abre a conta dos três caminhos (marketplace, loja própria e polo por success fee).
eyebrow: Comparativo · Canais de venda
pubDate: 2026-07-12
updatedDate: 2026-09-24
author: Equipe ROI Labs
faq:
  - q: 'Vale a pena vender no Mercado Livre ou na Shopee como fornecedor regional?'
    a: 'Depende do produto. O marketplace foi desenhado para item leve, padronizado, de alto giro e margem folgada, e cobra comissão na faixa de 10% a 20% conforme categoria e tipo de anúncio (confira a tabela vigente antes de qualquer conta). Para produto pesado, sob medida ou de margem fina, a comissão sozinha come a margem, e o frete termina o serviço.'
  - q: 'Por que o frete inviabiliza produto pesado ou volumoso no marketplace?'
    a: 'Porque a logística do marketplace é otimizada para pacote pequeno. Uma caixa de porcelanato, por exemplo, passa de 20 kg, e o piso de uma sala inteira vai de pallet. Some o programa de frete grátis subsidiado pelo vendedor acima de certo valor e a conta vira negativa. A venda local resolve isso com frete próprio ou retirada — é por isso que o cliente busca o produto com o nome da cidade.'
  - q: 'E a quebra e a devolução, de quem é o custo?'
    a: 'Do vendedor, na prática. No marketplace a política de devolução é do canal, não sua: o cliente abre a reclamação, você absorve o produto perdido, o frete de volta e o arranhão na reputação da conta. Numa venda local, o mesmo problema costuma virar uma troca resolvida no balcão.'
  - q: 'O que o marketplace resolve bem para um fornecedor regional?'
    a: 'Itens leves, padronizados e de alto giro — acessórios, consumíveis, reposição — e liquidação de ponta de estoque. Serve também como teste de demanda sem montar site. Só não confunda esse papel com o de canal principal do carro-chefe de margem.'
  - q: 'Qual a diferença entre vender no marketplace e vender pelo canal próprio?'
    a: 'No marketplace, o cliente é do canal: você aluga a audiência, disputa preço lado a lado com o concorrente e paga comissão sobre cada venda, para sempre. No canal próprio, o cliente é seu, a margem é cheia e o atendimento é consultivo — mas a captação passa a ser um problema seu, e é aí que a maioria dos fornecedores trava.'
  - q: 'Como o polo da ROI Labs se encaixa nessa comparação?'
    a: 'O polo é a terceira via: a ROI Labs constrói a captação de busca local do seu nicho (malha de páginas de alta intenção, catálogo, ferramentas, Google Shopping) e o pedido fecha no seu WhatsApp ou balcão — o cliente é seu. A remuneração é a anuidade simbólica da cadeira (R$ 2.640/ano) e um success fee sobre o que vendeu: não vendeu, não há success fee.'
  - q: 'Posso usar marketplace e canal próprio ao mesmo tempo?'
    a: 'Pode, e é a configuração mais saudável: marketplace para o item leve e de giro, canal próprio (ou polo) para o carro-chefe de margem, que é onde a venda consultiva local ganha. A única disciplina obrigatória é a coerência de preço e estoque entre os canais.'
  - q: 'Como eu faço a conta para o meu caso?'
    a: 'Pegue o ticket médio do seu carro-chefe e desconte, na ordem: comissão do canal, custo real do frete (peso e volume reais, não os do anúncio), taxa de pagamento e a provisão de quebra ou devolução. Compare o que sobra com a mesma venda feita localmente, com frete próprio e um success fee sobre o valor vendido. O simulador da ROI Labs roda essa segunda linha com qualquer percentual e volume.'
---

Marketplace é um canal excelente — para produto **leve, padronizado, de alto giro e margem folgada**. Para o fornecedor regional cujo carro-chefe é pesado, sob medida, de margem fina ou vendido com atendimento consultivo, a conta costuma não fechar: comissão de dois dígitos, frete que o canal não foi desenhado para carregar, devolução por sua conta e o cliente ficando com a plataforma. Antes de subir o catálogo no Mercado Livre ou na Shopee, vale abrir essa conta e comparar com as outras duas formas de vender: canal próprio e polo por success fee.

## Quanto o marketplace desconta de cada venda?

O marketplace desconta quatro coisas do seu ticket antes de você ver o dinheiro:

1. **Comissão** — na faixa de 10% a 20% conforme categoria e tipo de anúncio (confira sempre a tabela vigente do canal). Numa margem de revenda apertada, esse é o primeiro golpe e já costuma ser o fatal.
2. **Frete** — a logística do marketplace foi desenhada para pacote pequeno. No caso do porcelanato, uma caixa passa de 20 kg e o piso de uma sala inteira vai de pallet; com o programa de frete grátis subsidiado pelo vendedor acima de certo valor, a conta inverte de sinal. Qualquer produto volumoso ou frágil sofre a mesma lógica.
3. **Quebra e devolução** — a política de devolução é do canal, não sua: produto perdido, frete de volta e reputação arranhada ficam com você. No balcão, o mesmo problema é uma troca.
4. **Guerra de preço** — no marketplace o seu anúncio aparece ao lado do concorrente, ordenado por preço e reputação. Não existe atendimento consultivo, cálculo de quantidade ou orientação técnica. Existe centavo.

E há o custo que não aparece em nenhuma tabela: **o cliente não é seu**. Ele é do canal. A cada venda você paga comissão e devolve o relacionamento.

## O que o marketplace resolve bem?

O marketplace resolve bem itens **leves, padronizados e de alto giro** — acessórios, consumíveis, reposição —, além de liquidação de ponta de estoque e teste de demanda sem montar site. O erro não é usar marketplace; é usá-lo como canal principal do carro-chefe de margem.

## Onde está o comprador do fornecedor regional?

O comprador do fornecedor regional está buscando localmente, não navegando no marketplace nacional. Ele digita o produto com o nome da cidade, pergunta preço, calcula quantidade, quer tirar dúvida técnica e retirar na loja ou receber por frete local. Essa jornada é de **busca local de alta intenção**, e é o que o canal próprio captura: como isso funciona está em [como vender pela internet em Goiânia sem montar um e-commerce](/blog/vender-porcelanato-internet-goiania/) e em [o que é SEO programático](/blog/o-que-e-seo-programatico-pseo-revestimentos/).

O problema do canal próprio nunca foi o mérito — é o custo de construir a captação: plataforma, catálogo, integrações e, sobretudo, tráfego. A conta completa está em [e-commerce próprio vs polo pronto](/blog/ecommerce-proprio-vs-polo-pronto/) e os custos por canal em [quanto custa colocar uma empresa no Google em Goiânia](/blog/quanto-custa-loja-materiais-construcao-google-goiania/).

## Como os três canais se comparam?

| | Marketplace | Loja própria | Polo (success fee) |
| :-- | :-- | :-- | :-- |
| Custo antes da 1ª venda | Zero (só cadastro) | Plataforma + catálogo + integrações + tráfego | Só a anuidade da cadeira (R$ 2.640/ano) |
| Custo por venda | Comissão (10–20%) + frete + devolução | Taxas de pagamento e frete | % sobre a venda (em contrato) |
| De quem é o cliente | Do canal | Seu | Seu — fecha no seu WhatsApp ou balcão |
| Produto pesado, volumoso ou frágil | Ruim (frete e devolução) | Bom (frete local ou retirada) | Bom (frete local ou retirada) |
| Venda consultiva | Não existe | Sua | Sua — com páginas e ferramentas que qualificam o lead |
| Onde ganha | Item leve, giro, ponta de estoque | Marca própria com demanda | Busca local de alta intenção |

## Qual é a terceira via entre marketplace e loja própria?

O polo da ROI Labs inverte as duas trocas ruins do marketplace: **a captação é construída pela ROI Labs** (malha de páginas de alta intenção, páginas de produto, ferramentas como calculadora e comparador, feed no Google Shopping) e **o cliente continua sendo seu** — o pedido cai no seu WhatsApp ou balcão, com o seu atendimento. A remuneração é a anuidade simbólica da cadeira (R$ 2.640/ano) e um success fee sobre o que vendeu: sem CAPEX, e sem success fee em mês sem venda. Rode a sua conta no [simulador](/simulador/).

O modelo inteiro — exclusividade de cadeira por nicho, o que a ROI Labs assume e o que fica com o fornecedor — está na página do [modelo Growth Partner](/modelo/). Se a conta fechou, o passo seguinte é [candidatar sua empresa à cadeira do seu nicho](/#candidatar).

## Resumo

Marketplace cobra comissão de dois dígitos, carrega mal produto pesado ou frágil, joga a devolução na sua conta e fica com o seu cliente — defeitos que só não doem em item leve e de giro. Para o carro-chefe de margem, o jogo é **local, consultivo e de busca de alta intenção**. Use o marketplace onde ele é bom, e coloque o carro-chefe onde o seu comprador realmente está.
