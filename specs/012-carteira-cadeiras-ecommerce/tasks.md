...
- [ ] T048 [US3] Escrever a página da **primeira** cadeira e passar no verificador de T047. **CONTEÚDO ESCRITO E MEDIDO em 07/08 — falta só ligar.** Cadeira escolhida pelo Jean: `sirius` (não `atma`, que seria canibalizada). Preço e planos apurados no próprio produto, não estimados (`siriuscrm.com.br/pricing`, 07/08): Gratuito R$ 0 · Starter R$ 67 · **Pro R$ 147** · Business R$ 397. Medido com o flag temporariamente em `true`: **1817 palavras · 9 FAQ · preço no corpo · Offer ok** (piso: 800/6), `@graph` ÚNICO com `Organization, WebSite, Product, FAQPage`. 🚨 **Fica `publicado: false` e a task fica ABERTA porque o Sirius NÃO tem `CredencialGateway` (T033).** Publicar o botão sem gateway ligado é o próprio FR-008: venda feita, receita invisível. **Vira `true` quando T033+T034 fecharem, e só então** — é uma palavra de diff. Portão de FR-009 conferido nas duas posições: com `true` gera `/cadeira/sirius/` e entra no sitemap; com `false` não existe `dist/cadeira` e o sitemap não tem a URL
...
- [X] T052 [US5] Marcar `daCasa=true` em todas as cadeiras da casa e `exibirDaCasa=true` **apenas** em `sirius`, `meridian` e `orion` — a lista de exceções é **dado**, nunca condição no código (FR-010a). ✅ **A dúvida do fail-closed FECHOU 07/08: Jean confirmou `vertice` e `orcaobra` como parceiro externo → `daCasa: false` nos dois.** Saem do fail-closed e ENTRAM na régua do success fee. ⚠️ Está no **código**, não no banco: `app/src/lib/seats.ts` é o SEED, e produção só muda quando o seed rodar (ver T072b). Enquanto não roda, os dois seguem `daCasa=true` em prod — o lado que **sub-reporta**, que é o recuperável
- [ ] T072b ⚠️ **Rodar `npm run prisma:seed` (ou `npx prisma db seed`) em produção** e conferir no banco: (a) `vertice` e `orcaobra` com `daCasa=false` (T052); (b) `Fitas adesivas` com `estado='ocupada-vendavel'`. O (b) é um DEFEITO achado em 07/08 lendo `/api/cadeiras`: as 8 cadeiras de nicho nasceram antes da coluna `estado` e o laço de `DEFAULT_SEATS` no `prisma/seed.ts` só escrevia `ordem` — então a migração da 012 deixou todas no default `'vaga'` e **nenhuma rodada de seed as corrigia**. Em produção, `Fitas adesivas` servia `status: 'Ocupada · Tapepro'` com `estado: 'vaga'`, divergindo do skeleton no-JS e fazendo o mapa PERDER o `seat--taken` da Tapepro quando o JS rodava. O seed já foi corrigido; falta aplicá-lo
...
- [X] T057a [US5] ✅ **FEITA 07/08 — saída 2 (Jean): seção própria "A carteira" no institucional**, alimentada pelas cadeiras com `ordem >= 8 && siteUrl`, exibindo `rotulo` + `estado` + link. **SEM skeleton estático de propósito** — espelhar a lista em HTML foi a causa (a) abaixo, e repeti-la em 2× seria repetir o defeito; fonte única = `/api/cadeiras`, fetch falhou ⇒ seção não aparece. Zero backend, zero CSS novo (reusa `.map__grid`/`.seat`). O seletor do laço antigo virou `#cadeiras .map__grid .seat` porque agora há DOIS `.map__grid`. **Verificado no browser contra a API de produção** (não curl): 8 cards, `sirius`/`orion`/`meridian` como "Da casa" e os outros 5 como "Parceiro" — FR-010a mantida com `daCasa` sem vazar. Print: [snapshots/t057a-carteira.png](./snapshots/t057a-carteira.png) · ~~🚨 A carteira é INVISÍVEL no institucional — descoberto 07/08 pelo Jean ("não vi diferença em `roilabs.com.br`"), e nenhuma task acima cobre.** Duas causas somadas: (a) o skeleton estático de `site/src/pages/index.astro:28-37` espelha `DEFAULT_SEATS`, então o fetch reescreve **os mesmos valores** e a página renderiza igual com a API em 200 ou em 500; (b) o laço casa **por índice** sobre os 8 `<li>` do grid (`index.astro:219`) e a API devolve **16** cadeiras — as 9 de projeto têm `ordem` ≥ 8 e **nunca são renderizadas**. `estado`, `rotulo`, `siteUrl`, `produto` e `checkout` são servidos e lidos por ninguém. **T053/T054/T055 cumpriram FR-019 ao pé da letra e mesmo assim o objetivo não aconteceu.** ⚠️ **Decisão de produto antes de código** (3 saídas no handoff) — e ao exibir, usar `rotulo`, **nunca** `daCasa`: 4 das 9 são da casa sem exibição pública (FR-010a). O `ponytail:` em `index.astro:213` já nomeou o gatilho ("se a contagem divergir, casar por id"); ela divergiu~~
...
- [X] T058 [US4] ✅ **CONFIRMADO 07/08 pelo Jean: `loja.roilabs.com.br`** (o assumido). Um label só — o cert Universal da Cloudflare cobre apex + um nível, e `src/scripts/mapa-301.mjs` já recusa label de segundo nível. Destrava T059 e T061–T065
