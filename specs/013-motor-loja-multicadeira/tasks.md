...
- [x] T023 [US1] Rodar `git diff --name-only` e conferir que **apenas** `site-goiania/src/data/lojas.ts` e `site-goiania/src/data/teste-saas.ts` foram alterados. Qualquer outro arquivo é a feature reprovando SC-001
  - Decisão fechada em 08/08 (sessão 5), pelo Jean: **exceção aceita**, não reescrita de arquitetura. `contracts/loja-config.md` reescrito para refletir os 3 arquivos reais (2 espelhos de `lojas.ts` + 1 catálogo) — a duplicação servidor/site é deliberada (dois containers, dois deploys, ver comentário em `app/src/lib/lojas.ts`) e agora faz parte do contrato, com o teto de extração (`packages/lojas`) registrado. Também documentado: **não existe** teste de paridade automático entre os 2 espelhos hoje — ficou como lacuna conhecida, não foi criado nesta sessão.
