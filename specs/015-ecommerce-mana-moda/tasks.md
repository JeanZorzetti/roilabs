...
- [ ] T021 [US1] **BLOQUEADO** — Criar o registro DNS de `mana.roilabs.com.br` no **Cloudflare antes do deploy** e confirmar com `dig +short mana.roilabs.com.br`. ⚠️ Cert emitido contra NXDOMAIN **não se re-emite sozinho** — a 012 já pagou essa conta. Sem credencial de API do Cloudflare neste repo — precisa do Jean (manual no dashboard, ou token de API)
