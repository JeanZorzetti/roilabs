...
- [ ] **⚠️ GitHub Actions — CAUSA CONFIRMADA (banner na aba Actions, 2026-07-03): "A cobrança da sua conta está bloqueada. Atualize suas informações de pagamento."** Não é quota (0/2.000 min) nem o workflow: a CONTA está com billing travado (cartão vencido ou pagamento antigo falhado). Fix: `github.com/settings/billing` → Payment information → atualizar/regularizar o método de pagamento; o bloqueio cai e o `rank-tracking.yml` volta a rodar sozinho. **Sem pressa: o fallback local JÁ cobre** — task `roilabs-rank-tracking` no Task Scheduler (segunda 09:00, roda atrasado se a máquina estava off), validada de ponta a ponta (`37c2255`). Quando o Actions voltar: `Unregister-ScheduledTask roilabs-rank-tracking`.
