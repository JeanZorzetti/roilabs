...
- [ ] **⚠️ GitHub Actions do repo com `startup_failure` — NÃO é quota** (billing conferido 2026-07-03: 0/2.000 min usados, $0). Qualquer workflow (até `echo ok`) falha em 1s sem jobs. Próximo diagnóstico: abrir `github.com/JeanZorzetti/roilabs/actions` no navegador e procurar banner de aviso (contas free às vezes exigem verificação de método de pagamento pra Actions em repo privado); se nada, abrir ticket no GitHub Support. **Sem pressa: o fallback local JÁ cobre** — task `roilabs-rank-tracking` no Task Scheduler (segunda 09:00, roda atrasado se a máquina estava off), validada de ponta a ponta (`37c2255`). Quando o Actions voltar: `Unregister-ScheduledTask roilabs-rank-tracking`.
